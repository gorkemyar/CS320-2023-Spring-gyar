...
forall_to_foreach( forall: ('xs * ('x0 -> bool)) -> bool): ('xs * ('x0 -> unit)) -> unit =
  ...
  let
    val _ =
    forall(xs, fn(x0: 'x0) => (work(x0); true))
  in
  ( (*void*) )
...
fun
foreach_to_forall( foreach: ('xs * ('x0 -> unit)) -> unit): ('xs * ('x0 -> bool)) -> bool =
...
fun
foreach_to_foldleft( foreach: ('xs * ('x0 -> unit)) -> unit): ('xs * 'r0 * ('r0*'x0 -> 'r0)) -> 'r0 =
  fn(xs, r0, fopr) =>
    let
      val res = ref(r0)
    in
...
fun
foreach_to_length(foreach:('xs*('x0->unit))->unit): 'xs -> int =
  fn(xs) =>(foreach_to_foldleft(foreach)(xs, 0, fn(r0, x0) => r0 + 1))
...
fun
foreach_to_listize(foreach:('xs*('x0->unit))->unit): 'xs -> 'x0 list = (
  fn(xs) =>
  list_reverse(foreach_to_foldleft(foreach)(xs, nil, fn(r0, x0) => x0 :: r0)))
